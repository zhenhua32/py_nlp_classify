...
class SortedBatchSampler(Sampler):
    ...
    def __iter__(self):
        lengths = np.array(
            [(-l, np.random.random()) for l in self.lengths], dtype=[("sent_len", np.int_), ("rand", np.float_)]
        )
...
def batchify(batch):
    ...
    input_ids, input_mask, segment_ids, label_id, label_index = list(), list(), list(), list(), list()
...
class OCNLIProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        return self._create_examples(self._read_json(os.path.join(data_dir, "train.50k.json")), "train")
...
class MnliProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir, str_code):
        ...
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "dev_{}.tsv".format(str_code))), "dev_{}".format(str_code)
        )

    def get_test_examples(self, data_dir, str_code):
        """See base class."""
        return self._create_examples(
            self._read_tsv(os.path.join(data_dir, "test_{}.tsv".format(str_code))), "test_{}".format(str_code)
        )
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            label = tokenization.convert_to_unicode(
                line[-1] if set_type != "test_matched" and set_type != "test_mismatched" else "contradiction"
            )
            examples.append(InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
        return examples
...
class AXProcessor(DataProcessor):
    ...
    def get_test_examples(self, data_dir, str_code):
        """See base class."""
        return self._create_examples(self._read_tsv(os.path.join(data_dir, "diagnostic.tsv")), "diagnostic")
...
def convert_examples_to_features(
    args, examples, label_lists, max_seq_length, tokenizer, index, max_index, is_training=False
):
    ...
    features = pool.starmap(
        examples_to_features_worker,
        zip(
            examples,
            repeat(max_seq_length),
            repeat(tokenizer),
            repeat(label_map_lst),
            repeat(index),
            repeat(max_index),
            repeat(is_training),
            repeat(args),
...
def main():
    ...
    parser.add_argument(
        "--init_checkpoint", default=None, type=str, help="Initial checkpoint (usually from a pre-trained BERT model)."
    )
    parser.add_argument("--pretrain_model", default=None, type=str, help="Used to ontinue training.")
    parser.add_argument("--core_encoder", default="bert", type=str, help="core encoder, support 'bert' or 'lstm'.")
    parser.add_argument(
        "--do_lower_case",
        default=False,
        action="store_true",
    ...
    parser.add_argument(
        "--gradual_unfreezing", default=False, action="store_true", help="layerwise unfreezing gradient or not"
    )
    parser.add_argument("--debug", default=False, action="store_true", help="debug mode")
    parser.add_argument(
        "--add_dialog_token", default=False, action="store_true", help="add [SLR][BYR] token when process dialog data"
    )
    parser.add_argument(
        "--sequential", default=False, action="store_true", help="Different task will be sequential trained if True"
    )
    parser.add_argument("--lr_decay_factor", default=1, type=float, help="lr decay factor over all layers")
    parser.add_argument("--dropout", default=0.1, type=float, help="dropout during downstream task")
    parser.add_argument("--train_batch_size", default=32, type=int, help="Total batch size for training.")
    parser.add_argument("--eval_batch_size", default=8, type=int, help="Total batch size for eval.")
    parser.add_argument("--detach_index", default=-1, type=int, help="fix layers of transformer during finetune")
    ...
    parser.add_argument(
        "--save_checkpoints_steps", default=1000, type=int, help="How often to save the model checkpoint."
    )
    parser.add_argument("--save_model", default=False, action="store_true", help="save checkpoint or not")
    parser.add_argument("--no_cuda", default=False, action="store_true", help="Whether not to use CUDA when available")
    parser.add_argument("--local_rank", type=int, default=-1, help="local_rank for distributed training on gpus")
    parser.add_argument("--seed", type=int, default=42, help="random seed for initialization")
    parser.add_argument("--fast_train", default=False, action="store_true", help="sort examples by length")
    parser.add_argument(
    ...
    parser.add_argument(
        "--amp_type", default=None, type=str, help="whether to use mix precision, must in [O0, O1, O2, O3]"
    )
    ...
    if args.do_train:
        ...
        num_train_steps = int(
            sum([len(exs) for exs in train_examples])
    ...
    if args.do_train:
        ...
        if not args.fast_train:
            ...
            train_sampler = SortedBatchSampler(train_data.lengths(), args.train_batch_size, shuffle=not args.sequential)
    ...
    if args.do_eval:
        ...
        for task_name, processor in zip(args.task_name.split(","), processor_list):
            ...
            if task_name.lower() == "mnli":
                eval_examples.append(
                    processor.get_dev_examples(data_dir, "matched") + processor.get_dev_examples(data_dir, "mismatched")
                )
            else:
                if not args.add_dialog_token:
    ...
    if args.do_test:
        ...
        for task_name, processor in zip(args.task_name.split(","), processor_list):
            ...
            if task_name.lower() == "mnli":
                ...
                test_examples.append(
                    processor.get_test_examples(data_dir, "matched")
                    + processor.get_test_examples(data_dir, "mismatched")
                    + AXProcessor().get_test_examples(diagnostic_dir, "diagnostic")
                )
            else:
                if not args.add_dialog_token:
        ...
        for epoch_id in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                loss, _ = model(
                    ...
                    label_ids,
                    label_index,
                    epoch_id=epoch_id if args.gradual_unfreezing else -1,
                )
                if n_gpu > 1:
            ...
            if args.do_eval:
                ...
                eval_logits_files = [
                    open(os.path.join(args.output_dir, task_name + ".eval_logits_EP{}".format(epoch_id)), "w")
                    for task_name in task_name_list
                ]
                ...
                for input_ids, input_mask, segment_ids, label_ids, label_index in eval_dataloader:
                    ...
                    for index, (logit, label_id) in enumerate(zip(logits, np.split(label_ids, max_eval_index, axis=1))):
                        for is_match, single_label, ins_logit in zip(
                            (index == label_index).tolist(), label_id.tolist(), logit.tolist()
                        ):
                            if is_match:
                                eval_logits_files[index].write(str(single_label) + "\t" + str(ins_logit) + "\n")
                        if len(label_list[index]) != 1:
                            outputs = np.argmax(logit, axis=1)
                            tmp_eval_accuracy.append(
                                np.sum((outputs.flatten() == label_id.flatten()) * (index == label_index))
                            )
                            tmp_eval_corrcoef.append(([], []))
                            if len(label_list[index]) == 2 and task_name_list[index] == "cola":
                                TP = np.sum(
                                    (outputs.flatten() == label_id.flatten())
                                    * (outputs.flatten() == 1)
                                    * (index == label_index)
                                )
                                TN = np.sum(
                                ...
                                matthew_cnt[index] = (
                                    matthew_cnt[index][0] + TP,
                                    matthew_cnt[index][1] + TN,
                                    matthew_cnt[index][2] + FP,
                                    matthew_cnt[index][3] + FN,
                                )
                        else:
                            tmp_eval_accuracy.append(0)
                            batch_outputs, batch_label = [], []
                            for single_label, single_logit, flag in zip(
                                logit.flatten(), label_id.flatten(), index == label_index
                            ):
                                if flag:
                    ...
                    eval_corrcoef = [
                        (a[0] + b[0], a[1] + b[1]) for a, b, lst in zip(eval_corrcoef, tmp_eval_corrcoef, label_list)
                    ]
                ...
                with open(output_eval_file, "w") as writer:
                    ...
                    for key in sorted(result.keys()):
                        logger.info(
                            "%s:  %s:  %s = %s",
                            args.init_checkpoint if args.init_checkpoint is not None else args.pretrain_model,
                            args.task_name,
                            key,
                            str(result[key]),
                        )
            ...
            if args.do_test:
                ...
                instance_idx = 0
                output_test_files = [
                    open(os.path.join(args.output_dir, task_name + ".test_results_EP{}".format(epoch_id)), "w")
                    for task_name in task_name_list
                ]
                output_logits_files = [
                    open(os.path.join(args.output_dir, task_name.upper() + ".test_results_EP{}".format(epoch_id)), "w")
                    for task_name in task_name_list
                ]
                headers = ["id", "label"]
                ...
                for input_ids, input_mask, segment_ids, label_index in tqdm(test_dataloader):
                    ...
                    for index, logit in enumerate(logits):
                        ...
                        for is_match, predict_ins, labels, ins_logit in zip(
                            (index == label_index).tolist(), outputs, label_index.tolist(), logit.tolist()
                        ):
                            if is_match:
                                if len(label_list[index]) != 1:
                                    test_result[index].append((instance_idx, label_list[labels][predict_ins]))
                                    test_logits_result[index].append(
                                        (instance_idx, label_list[labels][predict_ins], ins_logit)
                                    )
                                else:
